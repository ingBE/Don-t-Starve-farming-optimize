...
if __name__ == "__main__":
    ...
    default_font.configure(family="malgunsl", size=15)

    spinbox_font = tkFont.Font(family="malgunsl", size=13)
    ...
    label1 = tk.Label(frame1, text="작물 종류", width = 10, bg="snow4")
    label1.pack(side="top")

    nameLabel = ["" for x in range(N)]
    for i in range(N):
        nameLabel[i] = tk.Label(frame1, text=NAMES[i], width=10, bg="snow3")
    ...
    label2 = tk.Label(frame2, text="씨앗 갯수", width=7, bg="snow4")
    ...
    label3 = tk.Label(frame3, text="추천 갯수", width=7, bg="snow4")
    ...
    empty1 = tk.Label(frame4, text="", height=2)
    empty1.pack()

    button1 = tk.Button(frame4, text="영양 계산", font=spinbox_font, width=7, command=calCompost)
    button1.pack()

    button2 = tk.Button(frame4, text="씨앗 수 계산", font=spinbox_font, width=7, command=optimize)
    button2.pack()
